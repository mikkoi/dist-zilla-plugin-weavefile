...
subtest_streamed 'Engine construction' => sub {
    T2->ok(
        dies { Dist::Zilla::Plugin::WeaveFile::Engine->new() },
        'dies without config_path',
    );
...
subtest_streamed 'Source resolution errors' => sub {
    ...
    T2->ok(
        dies { $engine->extract_pod_section('No::Such::Module', 'SYNOPSIS') },
        'dies for nonexistent module',
    );
    T2->ok(
        dies { $engine->extract_pod_section('no/such/file.pm', 'SYNOPSIS') },
